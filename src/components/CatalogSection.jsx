      ...
      <div className="container-shell">
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          ...
          <a
            ...
            aria-label="Open main KTM website in a new tab"
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {catalogs.map((catalog, groupIndex) => {
            const items = catalog?.items || [];
            const primaryHref = items[0]?.[1] || 'https://ktmleather.com/';

            return (
              <article
                key={catalog?.group || groupIndex}
                className="group rounded-2xl border border-stone-200 bg-white p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-teal/20 hover:shadow-card"
              >
                {/* Group Header */}
                ...
                <div className="mt-5 space-y-2.5">
                  {items.map(([label, href], itemIndex) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      aria-label={`Open ${label} catalogue in a new tab`}
                ...
                <div className="mt-5 border-t border-stone-200 pt-4">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={`Explore ${catalog?.group} collection`}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 max-w-2xl text-xs leading-5 text-stone-600 sm:text-sm">
                  Tell us the product category, article name, thickness,
                  finish, color and expected quantity and we can use the
                  relevant catalogue as the starting point for your sourcing
                  conversation.
                </p>
              </div>
            </div>
